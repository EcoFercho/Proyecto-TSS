...
def calcular_proyecciones(n_clicks, salario_mensual, meses_ahorro, tasa_crecimiento, contents):
    ...
    saldo_bajo_rendimiento = (salario_neto_inicial - gasto_mensual) * (((1 + r_3)**meses_ahorro - 1) / r_3)
    saldo_moderado = (salario_neto_inicial - gasto_mensual) * (((1 + r_7)**meses_ahorro - 1) / r_7)

    fig_comparacion = go.Figure()
    fig_comparacion.add_trace(go.Bar(x=["0% Simple", "3% Simple", "7% Simple"], y=[saldo_simple, saldo_bajo_rendimiento, saldo_moderado], name="Cálculo Simple", marker_color="royalblue"))
    fig_comparacion.add_trace(go.Bar(x=["3% EDO", "7% EDO"], y=[A_3[-1][0], A_7[-1][0]], name="Modelo Diferencial", marker_color="firebrick"))
    fig_comparacion.update_layout(title="Comparación de Ahorros", xaxis_title="Escenario", yaxis_title="Ahorro Acumulado (Bs.)", barmode='group')

    fig_evolucion = go.Figure()
    fig_evolucion.add_trace(go.Scatter(x=t, y=(salario_neto_inicial - gasto_mensual) * t, mode='lines', name='0% Simple', line=dict(color='green', dash='dash')))
    fig_evolucion.add_trace(go.Scatter(x=t, y=(salario_neto_inicial - gasto_mensual) * (((1 + r_3)**t) - 1) / r_3, mode='lines', name='3% Simple', line=dict(color='blue', dash='dash')))
    fig_evolucion.add_trace(go.Scatter(x=t, y=(salario_neto_inicial - gasto_mensual) * (((1 + r_7)**t) - 1) / r_7, mode='lines', name='7% Simple', line=dict(color='orange', dash='dash')))
    fig_evolucion.add_trace(go.Scatter(x=t, y=A_3.flatten(), name="3% EDO", line=dict(color='royalblue')))
    fig_evolucion.add_trace(go.Scatter(x=t, y=A_7.flatten(), name="7% EDO", line=dict(color='firebrick')))
    fig_evolucion.update_layout(title="Evolución del Ahorro", xaxis_title="Meses", yaxis_title="Ahorro Acumulado (Bs.)")
    ...
    fig_detalle.add_trace(go.Scatter(x=t, y=(salario_neto_inicial - gasto_mensual) * (((1 + r_3)**t) - 1) / r_3, name="Simple 3%", line=dict(dash='dot')), row=1, col=1)
